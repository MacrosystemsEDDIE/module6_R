...
In this assignment, we will generate forecasts of lake water temperature for 1-7 days into the future using a slightly more complex model than was used in the module6.Rmd. Rather than fitting a linear regression model between air temperature and water temperature, we will fit a multiple linear regression model using both today's water temperature and tomorrow's air temperature as predictors of tomorrow's water temperature. Then, we will explore how the forecast uncertainty of forecasts made using the multiple linear regression model compare to forecasts made using the simple linear regression model. This will involve the following steps:
...
##add in bit here about why new drivers would be expected to affect water temp; see MS EDDIE Module 1 visualizations for examples; look at powerpoint or bonus slides for powerpoint
