...
First, build data frame to fit model
...
ggplot() +
  ...
  geom_line(data = forecast_deterministic, aes(x = forecast_date, y = value, color = "Forecasted water temp.")) +
  geom_vline(xintercept = as_date(fc_date), linetype = "dashed") +
  ylab("Temperature (\u00B0C)") +
  theme_bw(base_size = 12) +
...
Build plot. Render plot - this should resemble the water temperature forecast plot in the R Shiny app, Activity B Objective 9 ("Both" model)

```{r}
ggplot() +
  geom_point(data = lake_obs, aes(x = date, y = wtemp, color = "Observed water temp.")) +
  geom_line(data = forecast_ensemble, aes(x = forecast_date, y = value, color = "Forecasted water temp.", group = ensemble_member), alpha = 0.6) +
  geom_vline(xintercept = as_date(fc_date), linetype = "dashed") +
  ylab("Temperature (\u00B0C)") +
  theme_bw(base_size = 12) +
...
**Question 8:** Generate parameter distributions based on parameters estimates for linear model.  
...
ggplot(data = example_distribution, aes(x = draws))+
  geom_histogram()+
  theme_classic()
```
...
**Question 9:**: Plot each of the parameter distributions you have created. 
...
**Question 10:** Adjust the forecasting for-loop to incorporate parameter uncertainty into your forecasts. Edit the code below where you see "INSERT YOUR CODE HERE". 
...
sd_df <- left_join(sd_df, sd_param, by = c("forecast_date"))
```
...
sd_df <- left_join(sd_df, sd_proc, by = c("forecast_date"))
```
...
**HINTS** Use code provided above as a starting point!

1) Randomly sample 30 values from our initial conditions distribution to use for our forecast
2) Set up a dataframe that we will fill with our ensemble water temperature forecasts
3) Fill the rows of the dataframe where `date == "2020-09-25"` with the 30 values you randomly sampled from your initial conditions distribution
4) Run forecast. This will be a for-loop looping through dates; see code above. Be sure you hold all sources of uncertainty besides initial conditions uncertainty constant; this means you will only use ONE ensemble member from the NOAA forecast, have fixed parameter values, and will not add process uncertainty.
5) Build and render plot
6) Calculate the standard deviation of each day of the forecast to quantify initial conditions uncertainty - we will use this information in the next section. Your final output should be a data frame with the following columns:
...
7) Left-join your dataframe with the standard deviation of initial conditions to the dataframe of standard deviations with all forecasts included; example code is provided below:
...
HINTS
1) Set up a dataframe that we will fill with our ensemble water temperature forecasts; see example code above.\
2) Fill the rows of the dataframe where `date == "2020-09-25"` with 30 values randomly sampled from your initial conditions distribution.\   
3) Run forecast. This will be a for-loop looping through dates; see code above. Be sure to include driver, process, initial condition, and parameter uncertainty.\   
4) Build and render a time series plot of your forecast; see example code above.\   
...
ggplot() +
  geom_bar(data = sd_df, aes(x = forecast_date, y = sd, fill = label), stat = "identity", position = "stack") +
