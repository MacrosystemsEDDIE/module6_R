...
driver_ensemble <- noaa_fc %>%
  pivot_wider(c(Forecast_date, Forecast_variable), names_from = Ensemble_member, values_from = value) 
...
forecast_df <- model_data %>%
  filter(Date >= "2020-09-22" & Date <= "2020-10-02") %>% #subset dates
  mutate(forecast = ifelse(Date == fc_date, wtemp, NA))  #create forecast column and set initial conditions to current water temperature
...
forecast_df <- forecast_df %>%
  mutate(fc_airt = ifelse(Date > fc_date, mem01$value, NA)) #insert airtemp fc values here
...
for(i in fc_days) {
  forecast_df$forecast[i] <- forecast_df$fc_airt[i] * coeffs[2] + forecast_df$forecast[i-1] * coeffs[3] + coeffs[1]
 } 
...
curr_wt <- lake_df %>% #pull current water temperature to be initial condition
  filter(Date == fc_date) %>%
  select(wtemp)

#build forecast data frame
forecast_ensemble <- tibble(Date = rep(seq.Date(from = ymd(fc_date), to = ymd("2020-10-02"), by = "day"), times = n_members),
                            Ensemble_member = rep(1:30, each = 8),
                            Forecast_variable = "water_temperature",
                            prediction = NA) %>%
  # insert today's water temperature as initial condition
  mutate(prediction = ifelse(Date == fc_date, curr_wt$wtemp, NA))
