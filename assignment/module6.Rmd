...
11. Build and fit your new model.
12. Generate forecasts with partitioned uncertainty using your new model.
13. Compare uncertainty contributions between your new model and the previous model.
...
A linear relationship is a statistical term used to describe a straight-line relationship between two variables. Linear relationships can be expressed either in a graphical format where the variable and the constant are connected via a straight line or in a mathematical format where the independent variable is multiplied by the slope coefficient, added by a constant, which determines the dependent variable. A *multiple linear regression* includes more than one independent variable, each multipled by its own coefficient.

In our example, water temperature is the dependent variable and lagged water temperature and air temperature are the independent variables.
...
A **for-loop** runs a section of code repeatedly. The number of times the for-loop runs is specified by the coder, either by specifying a number of times the code should be run, or by specifying a condition that must be met before the for-loop stops. For example, below we write a for-loop that prints "Hello World!" eight times.
```{r}
for(i in 1:8){
  print("Hello World!")
}
...
Then, we create an empty vector in which we will store our starting water temperature as well as our predicted water temperatures for the next seven days (so total length of vector = 8 days).
...
Of course, this is probably not a very good model for predicting water temperature, because it would lead to water temperature increasing infinitely into the future! Now, we will use the concept of a for-loop to make a forecast of water temperature using the linear model you fit above.
...
$$WaterTemp_{t+1} = \beta_1 + \beta_2*WaterTemp_t + \beta_3*AirTemp_{t+1}$$
But we know that water temperature can be affected by other processes as well (such as rain, inflow streams to a lake, or water column mixing, to name a few) and that our model has simplified or ignored these. To account for the uncertainty these simplifications introduce to our model, we can add in process noise (W) to our model at each time step. In this model, water temperature tomorrow is equal to today's water temperature plus tomorrow's forecasted air temperature plus some noise (W),
...
Build time series plot of lake data. In addition to air and water temperature, we now have wind and solar radiation data as well.
...
## 12. Generate forecasts with partitioned uncertainty

Below, you will generate forecasts the incorporate **driver**, **parameter**, **process**, and **initial conditions** uncertainty. These forecasts will be needed to compare how forecast uncertainty differs between your new model and the previously fitted model.
...
**Q.21** Use the `rnorm()` function and the information in the `coeffs` and `params.se` objects to adapt the code below to generate parameter distributions for each of the parameters in your multiple regression model. *Hint:* you may need to add additional columns, (`beta4`,`beta5`...), to the dataframe and populate them with draws from a normal distribution.
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model using parameter distributions instead of fixed values - REVISE THIS!!
...
## 13. Compare uncertainty across models.
...
Plot the contribution of each source of uncertainty to total forecast uncertainty for both your model and the previously fitted model.
...
**Answer Q.26** 
Compare amount of total forecast uncertainty over time:    
