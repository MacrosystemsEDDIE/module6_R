...
- **ensemble_member**: this is an identifier for each member of the 31-member ensemble.   
...
Setting up an empty dataframe that we will fill with our water temperature predictions. Notice that this dataframe is much longer than the `forecast_deterministic` dataframe because we are now forecasting with 31 ensemble members.
...
With traditional modelling efforts, people generally find one set of the 'best fit' parameters and use them to predict with their model. 
...
Using parameter distributions allows for a better representation of the potential predicted outcomes, leading to better quantification of the uncertainty. Run the following code chunk and examine the slides to learn more about parameter uncertainty.

```{r, echo=FALSE, out.width = '80%'}
knitr::include_graphics("./images/param_uc_slides/Slide1.png")
knitr::include_graphics("./images/param_uc_slides/Slide2.png")
knitr::include_graphics("./images/param_uc_slides/Slide3.png")
knitr::include_graphics("./images/param_uc_slides/Slide4.png")
knitr::include_graphics("./images/param_uc_slides/Slide5.png")
knitr::include_graphics("./images/param_uc_slides/Slide6.png")
knitr::include_graphics("./images/param_uc_slides/Slide7.png")
knitr::include_graphics("./images/param_uc_slides/Slide8.png")
knitr::include_graphics("./images/param_uc_slides/Slide9.png")
```
...
**Process uncertainty** is the uncertainty caused by our inability to model all processes as observed in the real world. Run the following code chunk and examine the slides to learn more about process uncertainty.
...
**Initial conditions** uncertainty refers to uncertainty arising because the initial conditions are not measured or are imprecisely measured. Run the following code chunk and examine the slides to learn more about initial conditions uncertainty.
...
Group the dataframe by date and the type of uncertainty included in the forecast, then calculate the variance across all 31 ensemble members for each date and uncertainty type. **Notice** that variance has units of degrees Celsius squared.

```{r}
variance_df <- all_forecast_df %>%
...
```{r}
plot_partitioned_uc(variance_df)
```
...
**HINT** The code `+ rnorm(n = n_members, mean = 0, sd = sigma)` is what adds process uncertainty to your model - don't accidentally delete this!
...
**Q.24** Adjust the forecast code below under `#run model` to run your unique model with initial conditions uncertainty.
...
```{r}
plot_partitioned_uc(variance_df2)
plot_partitioned_uc(variance_df)
```
